...
class TranslateUschemaFreebase(TranslateUschemaFb15k):
  ...
  def generator(self, data_dir, tmp_dir, train):
    ...
    source_vocab_file = '%s/vocab.uschema_fb_source' % self.root_dir
    target_vocab_file = '%s/vocab.uschema_fb_target' % self.root_dir
...
class TranslateUschemaFreebaseKb(TranslateUschemaFreebase):
  ...
  def generator(self, data_dir, tmp_dir, train):
    root_dir = '/iesl/data/clueweb_2016_full/freebase_ep_subset/2500k_ep_subset_with_kb'
    tag = "train" if train else "dev"
    source_path = '%s/%s' % (root_dir, tag)

    source_vocab_file = '%s/vocab.uschema_fb_kb_source' % root_dir
    target_vocab_file = '%s/vocab.uschema_fb_kb_target' % root_dir
    source_vocab = text_encoder.TokenTextEncoder(source_vocab_file)
    target_vocab = text_encoder.TokenTextEncoder(target_vocab_file, replace_oov='<UNK>')
